...
__Accuracy__  
Rate at which the model is able to predict the correct value.
\begin{equation}
...
Increasing the model complexity will reduce the bias but increase the variance. Simplifying a model will see the two trends reversing. In the end, the lowest total error is a trade-off between bias and variance.
